---
name: capcut-recipe-video-editor
description: |
  요리/레시피 원본 영상 1개 + 엔딩 이미지만 넣으면(레시피 텍스트 .md는 선택), 로컬 Whisper로 자막 생성 → 무자막 구간 컷 → 60초 하이라이트 선별(.md가 있으면 식재료 등장 구간 중심, 없으면 순서대로) → (선택) 레시피 오버레이 → 자막 스타일링/확대 → (선택) 식재료 등장 시점 효과음 → 엔딩 이미지까지 전부 자동 처리해서 CapCut(macOS)에서 바로 열어 검토·수정할 수 있는 draft를 만드는 pycapcut 기반 자동화 시스템을 처음부터 구축하는 스킬. CLI와 파일 드래그드롭 지원 로컬 웹 대시보드(FastAPI) 둘 다 만든다. 검증된 코드 템플릿(`templates/`)과 macOS CapCut 연동 과정에서 실제로 겪은 함정·해결책이 전부 포함되어 있어, 처음부터 다시 디버깅하지 않고 바로 동작하는 시스템을 만들 수 있다.
  ...
  Use when: 사용자가 요리/레시피 숏폼 영상을 pycapcut+Whisper 기반으로 자동 편집하는 시스템을 처음 구축하려고 할 때. 이미 이 스킬로 만든 시스템이 있다면 이 스킬을 다시 쓰지 말고 기존 프로젝트를 직접 수정한다.
allowed-tools: Read, Write, Edit, Bash, AskUserQuestion
---

# capcut-recipe-video-editor 스킬
...
새 프로젝트 폴더(사용자 워크스페이스의 Johnny Decimal 규칙 등 기존 컨벤션이 있으면 그걸 따른다)에 이 스킬의 `templates/recipe_pipeline/`과 `templates/webapp/`, `templates/requirements.txt`를 그대로 복사한다:
...
`templates/recipe_pipeline/`은 이미 완성된 10단계 파이프라인이다 (아래 "파이프라인 단계" 참고). 사용자의 실제 영상 1개 + 레시피 .md + 엔딩 이미지로 CLI를 끝까지 돌려본다:

```bash
python3 -m recipe_pipeline.cli \
  --video 영상.mp4 --content-md 레시피.md --end-image 엔딩.jpg \
  --draft-name test-1
```

중간 산출물(SRT, `pipeline_log.json`)을 확인하고, CapCut에서 결과 draft를 열어 무자막 컷·60초 하이라이트·오버레이·자막 스타일·효과음·엔딩 이미지가 전부 반영됐는지 사용자에게 확인받는다.
...
`http://127.0.0.1:8000`에서 파일 드래그드롭 → 실시간 진행상황 → 완료 요약까지 확인한다.

## 파이프라인 단계 (원래 사용자 요청 순서, 실제 실행 순서와 다름)

사용자가 이 시스템을 요청할 때 보통 아래 순서로 설명하지만, **4단계(60초 선별)는 5단계(식재료 구간 분석) 결과가 있어야 계산 가능**하므로 실제 코드 실행 순서는 1→2→3→[5]→4→6→7→8→9→10이다. `pipeline.py`의 `STAGE_LABELS`와 로그에 이 재배열이 이미 명시되어 있다.

1. 영상 삽입
2. 자막 스크립트 생성 (로컬 Whisper)
3. 무자막 구간 컷 편집
4. 총 60초 클립 편집 (식재료 등장 구간 중심)
5. 자막 기반 식재료 사용 구간 분석
6. 레시피 .md 내용을 오버레이로 추가
7. 자막 글꼴/크기/위치/확대
8. 식재료 등장 시점마다 자동 효과음
...
### 5. "무자막 컷"과 "60초 선별"은 gap-merge를 먼저 하면 안 된다
사람이 자연스럽게 말하는 것과 달리 연속적으로 읽는 오디오(TTS 등)는 문장 사이 pause가 거의 없다. gap으로 먼저 발화 블록을 뭉쳐놓고 그 블록 단위로 60초를 선별하면, 전체가 하나의 거대한 블록이 되어 예산(60초)을 넘겨 통째로 탈락하는 문제가 생긴다. **cue(구문) 단위로 먼저 선별**하고(`stage_highlight.select_highlight_cues`), 선별된 cue들 중 원본에서 가까운 것끼리만 나중에 하나의 클립으로 묶어야(`stage_gaps.merge_cues_into_blocks`) 이 문제가 생기지 않는다.

### 6. 식재료 감지용 .md 형식은 최대한 관대하게 파싱해야 한다
실사용자가 준 .md 파일은 `## 재료`가 아니라 `# ... 재료 ...`(레벨 무관, "재료"라는 단어만 포함) 헤딩이었고, 불릿이 아니라 **마크다운 표**(`| 채소 | 수량 | 금액 |`)였다. 정확히 `## 재료`만 인식하는 파서는 이런 실제 파일에서 완전히 실패해서 오버레이·효과음이 0개로 조용히 안 만들어진다. `md_parsing.py`는 헤딩을 `#{1,6}.*재료.*`로 느슨하게 매칭하고, 불릿 목록과 마크다운 표 둘 다 지원하며, 표는 헤더 행(구분선 `|---|---|` 이전 행)을 자동으로 건너뛴다. 그리고 "다진 마늘"처럼 재료명 앞에 수식어가 붙은 경우 수식어(`_MODIFIER_PREFIXES` 목록)를 걸러내고 핵심 명사만 키워드로 쓴다.
...
- [ ] 실제 영상으로 CLI 끝까지 실행 성공
- [ ] CapCut에서 결과 draft 확인: 영상/자막/오버레이/효과음/엔딩이미지 전부 반영, 클립 삭제해도 안 깨짐
- [ ] 같은 draft 이름으로 2번 이상 재실행해도 정상적으로 열리는지 (함정 #3 회귀 확인)
- [ ] 사용자의 실제 .md 형식(표든 불릿이든)에서 재료가 정상 감지되는지
- [ ] 웹 대시보드에서 파일 업로드 → 실행 → 완료까지 브라우저로 실제 확인

## 파일 구조

```
capcut-recipe-video-editor/
├── SKILL.md                          # 이 파일
└── templates/
    ├── requirements.txt              # pycapcut, faster-whisper, fastapi 등
    ├── recipe_pipeline/               # 코어 파이프라인 (CLI 포함)
    │   ├── cli.py
    │   ├── config.py                 # PipelineConfig — 모든 조절 가능한 파라미터
    │   ├── pipeline.py               # orchestrate() — 전체 단계 순서 조정
    │   ├── stage_*.py                # 단계별 모듈 (파일명이 곧 역할)
    │   ├── md_parsing.py             # .md에서 재료 목록 추출 (표+불릿 지원)
